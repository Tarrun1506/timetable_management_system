...
const AdminSidebar = ({ activeTab, onTabChange, showQuickActions = true, userRole = 'admin' }) => {
  ...
  return (
    <>
      {/* Mobile Menu Button - Fixed position, only visible on mobile/tablet */}
      <button
      ...
      {/* Sidebar */}
      <aside
        className={`${isCollapsed ? 'w-20' : 'w-72'} rounded-2xl border-r shadow-2xl transition-all duration-300 ease-in-out
          ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}
          lg:sticky lg:top-16
          fixed top-0 left-0 bottom-0 z-40 lg:z-auto
          ${isDarkMode
      ...
      >
        ...
        {/* Hidden on mobile since sidebar is an overlay */}
        <div className={`absolute top-4 z-20 -right-4 hidden lg:flex items-center justify-center`} style={{ pointerEvents: 'none' }}>
          ...
          <button
            ...
            className={`absolute p-2 rounded-full shadow-lg transition-all duration-300 transform hover:scale-105 border focus:outline-none focus:ring-2 focus:ring-offset-2 ${isDarkMode ? 'bg-gradient-to-br from-blue-700 to-purple-700 text-white border-transparent' : 'bg-white border-gray-200 text-gray-700'
              }`}
          ...
          <nav className="space-y-1 mb-2">
            ...
            {navigationTabs.map((tab) => (
              ...
              >
                ...
                <tab.icon className={`${isCollapsed ? 'w-6 h-6' : 'w-5 h-5'} ${currentTab === tab.id ? 'text-white' : ''
                ...
                {quickActions.map((action, index) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center ${isCollapsed ? 'justify-center' : 'space-x-3'} ${isCollapsed ? 'px-3' : 'px-4'
                        } py-2 rounded-lg transition-all duration-200 text-white ${action.color} ${isActive ? 'ring-2 ring-white ring-opacity-50 shadow-xl scale-[1.02]' : 'shadow-md hover:shadow-lg hover:scale-[1.02]'
                        } group`}
